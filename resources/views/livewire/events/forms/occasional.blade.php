<form wire:submit="validateDraft" class="space-y-6">
    <x-card header="Dados do evento" shadowless bordered>
        <div class="grid gap-4 md:grid-cols-2">
            <x-select.native wire:model="group_id" label="Grupo organizador *" required>
                <option value="">Selecione</option>
                @foreach ($groups as $group)
                    <option wire:key="event-group-{{ $group->id }}" value="{{ $group->id }}">{{ $group->name }}</option>
                @endforeach
            </x-select.native>
            ...
            <x-input wire:model="name" label="Nome *" maxlength="255" required class="md:col-span-2" />
    ...
    @if ($advertisable)
        <x-card header="Detalhes" shadowless bordered>
            <div class="grid gap-4 md:grid-cols-2">
            <x-input wire:model="subtitle" label="Subtítulo" maxlength="255" class="md:col-span-2" />
    ...
    <x-card header="Ambientes" shadowless bordered>
        <div class="space-y-4">
            <x-select.styled wire:model.live="community_id" label="Comunidade" :options="$communities"
            ...
            @foreach ($selectedPlaces as $place)
                <section wire:key="event-place-{{ $place->id }}" class="grid gap-4 rounded-lg border border-gray-200 p-4 dark:border-gray-700 md:grid-cols-3">
