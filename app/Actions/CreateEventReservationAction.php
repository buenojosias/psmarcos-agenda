...
class CreateEventReservationAction
{
    public function __construct(private CheckPlaceAvailabilityAction $checkPlaceAvailability) {}

    /** @param array<string, mixed> $data */
    public function handle(Event $event, array $data, User $user): PlaceReservation
    {
        return DB::transaction(function () use ($event, $data, $user): PlaceReservation {
            ...
            $hasReservations = PlaceReservation::query()
                ->where('event_id', $event->id)
                ->lockForUpdate()
                ->exists();

            if (! $hasReservations && ! $this->coversEvent($event, $reservedFrom, $reservedTo)) {
                throw ValidationException::withMessages([
                    'reserved_from' => 'A primeira reserva deve cobrir integralmente o período do evento.',
                ]);
            }

            $this->ensureAvailable($place, $reservedFrom, $reservedTo);

            return PlaceReservation::query()->create([
                'event_id'      => $event->id,
                'place_id'      => $place->id,
                'reserved_from' => $reservedFrom,
                'reserved_to'   => $reservedTo,
                'is_primary'    => ! $hasReservations,
            ]);
        });
    }
    ...
    private function ensureAvailable(Place $place, CarbonImmutable $reservedFrom, CarbonImmutable $reservedTo): void
    {
